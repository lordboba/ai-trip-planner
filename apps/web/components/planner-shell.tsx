...
export function PlannerShell({
  ...
  const importIcs = useCallback(async (file: File) => {
    ...
    try {
      const imported = await tripPlannerApiClient.importIcsCalendar({
        file,
        startDate,
        endDate,
      }) as ImportedCalendar;
  ...
  const connectGoogle = useCallback(async () => {
    ...
    try {
      const data = await tripPlannerApiClient.connectGoogleCalendar({ startDate, endDate });
  ...
  useEffect(() => {
    ...
    const importGoogle = async () => {
      ...
      try {
        const imported = await tripPlannerApiClient.importGoogleCalendar({
